...
var sortedByAlphabet = function (array) {
...
var sortedSkills = sortedByAlphabet(skills);

console.log('Массив отсортированных по алфавиту скиллов = ', sortedSkills);




  var  people = _.map(obj, function(str) {
    return { "name": str.name, "friends": str.friends.length }
  });

  var names = _.map(_.sortBy(people, "friends"), "name");

  console.log('\n2. Массив "Кто самый дружелюбный?": \n', names);



  var allFriends = _.sortedUniq(_.map(_.sortBy(_.flatten(_.map(obj, "friends")), "name"), "name"));

  console.log("Все друзья представленных пользователей:", allFriends);
